...
function TokenStream(input) {
  var current = null;
  var keywords = " let if then else lambda 位 true false ";
  return {
      next  : next,
      peek  : peek,
      eof   : eof,
...
const code = 'bar = lambda(){println("foo 里面呢"); println("foo 里面呢"); println("foo 里面呢"); 1 + 2; };bar(); foo = lambda(return){println("foo 里面呢");return("DONE");println("bar");};CallCC(foo);';
// const code = `
// fail = lambda() false;
